...
class DDOS_API  
  ...
  #this class variable stores the local ip address
  @@local_ip
  ...
  def config_api(failover_filename="failover.txt", failover_timeout)
    #set the failover timeout to failover_timeout
    @@failover_timeout = failover_timeout

    #start with a private method to get my local ip address
    ...
    while line = failover.gets do
      ...
      if(ip == @@local_ip || ip == 'localhost') then
          line = failover.gets
          if(line == nil) then
              failover.rewind
              line = failover.gets
  ...
  def old_ddos_check(timestamp)
    begin
      ...
      timeout(@@failover_timeout) do
        ...
        length = @@receive_connections.length -1
